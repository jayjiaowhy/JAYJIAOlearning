# CS/DS 总索引

这个索引用于汇总计算机科学与数据科学相关课程。原始笔记仍保留在各自目录里，这里负责导航、资源清点和概念入口。

## 学习主线

- 入门与工具：[[cs_ds/cs_introduction/00-cs_introduction索引|CS 入门与衔接]] -> [[cs_ds/cs_self_learning/00-cs_self_learning索引|CS 自学资料库]] -> [[cs_ds/python/00-python索引|Python]]。
- 编程与实现：[[cs_ds/程序设计C++/00-程序设计C++索引|C++ 程序设计]] -> [[cs_ds/数据结构与算法/00-数据结构与算法索引|数据结构与算法]]。
- 理论与系统：[[cs_ds/离散数学/00-离散数学索引|离散数学期末复习]] -> [[cs_ds/数据结构与算法/00-数据结构与算法索引|算法与复杂度]] -> [[cs_ds/计算机原理/00-计算机原理索引|计算机原理]] -> [[cs_ds/数据库/00-数据库索引|数据库]]。
- 智能与数据：[[cs_ds/人工智能/00-人工智能导论索引|人工智能导论]] -> [[cs_ds/数科导/00-数科导索引|数科导]] -> [[cs_ds/机器学习/00-机器学习索引|机器学习]] -> [[cs_ds/推荐系统/00-推荐系统索引|推荐系统]] -> [[cs_ds/数据挖掘/00-数据挖掘索引|数据挖掘]]。

## 核心笔记

- [[cs_ds/cs_introduction/cs自学指南|CS 自学指南]]
- [[cs_ds/cs_self_learning/docs/CS学习规划|CS 学习规划]]
- [[cs_ds/Discrete-Mathematics/离散数学复习重点|离散数学复习重点]]
- [[cs_ds/离散数学/00-离散数学索引|离散数学期末复习]]
- [[cs_ds/计算机原理/00-计算机原理索引|计算机原理]]
- [[cs_ds/数据结构与算法/00-数据结构与算法索引|数据结构与算法]]
- [[cs_ds/数据结构与算法/算法设计课程书籍笔记/算法终极复习|算法设计终极复习：DP、网络流、NP-Complete]]
- [[cs_ds/数据库/00-数据库索引|数据库]]
- [[cs_ds/人工智能/2knowledge and search|Knowledge and Search]]
- [[cs_ds/数科导/2.数据获取与爬虫|数据获取与爬虫]]
- [[cs_ds/数科导/4.数据可视化|数据可视化]]
- [[cs_ds/机器学习/00-机器学习索引|机器学习]]
- [[cs_ds/推荐系统/00-推荐系统索引|推荐系统：召回、排序与线上实验]]

## 资料清单

- Markdown：各课程索引与主笔记集中在 `cs_ds/cs_introduction/`、`cs_ds/cs_self_learning/`、`cs_ds/Discrete-Mathematics/`、`cs_ds/人工智能/`、`cs_ds/机器学习/`、`cs_ds/推荐系统/`、`cs_ds/数据库/`、`cs_ds/数据结构与算法/`、`cs_ds/数科导/`、`cs_ds/程序设计C++/`。其中数科导新增的 `data100/` 六篇中文教程覆盖生命周期、表格数据、数据理解、其他数据源、线性建模、分类与优化。
- PDF/PPT：Python 教材、数据库课件、数据结构与算法 PDF、计算机组成原理作业与实验、数据挖掘周课件、C++ 课程资料；机器学习目录新增深度学习、统计学习、机器学习数学与竞赛解析资料；推荐系统目录新增王树森课程及“概念—召回—排序—交叉—序列—多样性—冷启动—指标”八部分课件。
- Office：数据挖掘 Jupyter 安装说明与 C++ 作业报告。
- 代码/数据：数据挖掘 notebook 和 CSV 数据、C++ 作业源码。

## 高频概念

- [[00-Wiki/Concepts/复杂度分析|复杂度分析]]
- [[00-Wiki/Concepts/数据结构|数据结构]]
- [[00-Wiki/Concepts/数组与串|数组与串]]
- [[00-Wiki/Concepts/链表|链表]]
- [[00-Wiki/Concepts/栈与队列|栈与队列]]
- [[00-Wiki/Concepts/图算法|图算法]]
- [[00-Wiki/Concepts/动态规划|动态规划]]
- [[00-Wiki/Concepts/网络流|网络流]]
- [[00-Wiki/Concepts/数据库系统架构|数据库系统架构]]
- [[00-Wiki/Concepts/知识表示|知识表示]]
- [[00-Wiki/Concepts/机器学习基础|机器学习基础]]
- [[00-Wiki/Concepts/逻辑回归|逻辑回归]]
- [[00-Wiki/Concepts/神经网络|神经网络]]
- [[00-Wiki/Concepts/偏差方差|偏差方差]]
- [[00-Wiki/Concepts/探索性数据分析|探索性数据分析]]
- [[00-Wiki/Concepts/数据可视化|数据可视化]]
- [[00-Wiki/Concepts/分类模型|分类模型]]
- [[00-Wiki/Concepts/计算机组成原理|计算机组成原理]]
- [[00-Wiki/Concepts/存储器层次|存储器层次]]
- [[00-Wiki/Concepts/NP完全性|NP 完全性]]
- [[00-Wiki/Concepts/强化学习|强化学习]]
- [[00-Wiki/Concepts/协同过滤|协同过滤]]
- [[00-Wiki/Concepts/双塔模型|双塔模型]]
- [[00-Wiki/Concepts/学习排序|学习排序]]
- [[00-Wiki/Concepts/A-B测试|A/B 测试]]

## 复习抓手

- 先用语言和工具建立实现能力，再用数据结构和算法建立抽象能力。
- 数据库、AI 和数据挖掘分别对应数据管理、知识推理和数据建模。
- 大型资料库优先走课程索引，不直接从文件树硬翻。
- 推荐系统复习按“目标指标 -> 召回 -> 粗排/精排 -> 重排与多样性 -> 实验验证”走；模型改进是否上线要回到 A/B 测试与留存等业务指标判断。
- 算法设计期末复习优先从数据结构与算法索引进入，再按续命丹、终极复习和模拟卷回看。
